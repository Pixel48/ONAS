...
int main() {
  sf::RenderWindow window(sf::VideoMode(VW, VH), "ONAS Project",
  ...
  zTexture.loadFromFile("img.m/z�batka (1).png");
  z.setPosition(0, 0);
  ...
  dTexture.loadFromFile("img.m/D�wi�k.png");
  d.setPosition(0, 0);
  ...
  paper.setTextureRect(
      sf::IntRect(5 * paperSize.x, 0, paperSize.x, paperSize.y));
...
#ifdef DEV
    if(sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
      text.setPosition(sf::Mouse::getPosition(window).x,
                       sf::Mouse::getPosition(window).y);
    }
#endif
